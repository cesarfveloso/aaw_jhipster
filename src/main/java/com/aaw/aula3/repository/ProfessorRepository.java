...
public interface ProfessorRepository extends JpaRepository<Professor, Long> {
    @Query("select distinct professor from Professor professor left join fetch professor.professors")
    List<Professor> findAllWithEagerRelationships();

    @Query("select professor from Professor professor left join fetch professor.professors where professor.id =:id")
